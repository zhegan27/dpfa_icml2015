...
load 'wiki_test'; [~,Ntest] = size(wordsTest);

% Setup
P = 7702;

% get 10000 documents for inialization
Ntrain = 3500*100;
fid_train = fopen('wiki_train.txt', 'r');
wordsTrain = GetBatch(fid_train, Ntrain, 'wiki_train.txt');
fclose(fid_train);
...
%%
[llik_mean,C_d] = nHDP_test(a ,Xcnt_test,Xid,Xcnt,Tree,beta0);
loglike = llik_mean./sum(sum(wordsTest(:,1:Ntest)));
perp = exp(-loglike);

save('wiki_nHDP_02022015','Tree','perp');
