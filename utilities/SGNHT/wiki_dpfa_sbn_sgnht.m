...
% Setup
N = 1000000;
[P, Ntest] = size(wordsTest); wordsTest0 = wordsTest;
Ns = 100;
K = [128, 64, 32];
nlayer = length(K);
...
% options
burnin = 3500; collection = 1500; step = 1;
...
for iter = 1:burnin + collection
	...
	Xtrain_s = GetBatch(fid_train, Ns, 'wiki_train.txt');
    Yflagtrain_s = Xtrain_s > 0;
	
    Theta = 1/K(1)*ones(K(1), Ns);
    
    prob = 1./(1+exp(-b));
    H{nlayer} = +(repmat(prob, 1, Ns) > rand(K(nlayer), Ns));
    for nl = nlayer-1:-1:3
    ...
    gradPhi = N * gradPhi / Ns / ns + (eta - Phi_);
	...
	steps = max(1e-8, (1 + iter)^(-0.33))/100;
    steps1 = steps / 1000;
